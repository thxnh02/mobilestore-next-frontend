...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const product = Number.isFinite(productId) ? await api.getProductById(productId).catch(() => null) : null;

  if (!product) {
    return (
      <div className="container">
        <EmptyState title="Không tìm thấy sản phẩm" message="Sản phẩm có thể đã ngừng kinh doanh hoặc tạm thời chưa hiển thị." href="/products" action="Quay lại danh sách" />
      </div>
    );
  }
